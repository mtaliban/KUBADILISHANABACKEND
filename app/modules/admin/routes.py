...
async def list_users(_=Depends(current_admin),
    ...
    if q: qd["$or"] = [{"full_name": {"$regex": _escape_regex(q), "$options": "i"}}, {"phone_primary": {"$regex": _escape_regex(q)}}, {"phone_alt": {"$regex": _escape_regex(q)}}, {"email": {"$regex": _escape_regex(q), "$options": "i"}}]
