"""In-memory WebSocket registry per user (single-process; for multi-worker use Redis pub/sub)."""
...
class ConnectionManager:
    ...
    async def send_to_user(self, user_id: str, payload: dict) -> int:
        """Broadcast to every open socket for the user. Returns delivered count."""
        conns = self._active.get(user_id)
        if not conns:
            return 0
        delivered = 0
